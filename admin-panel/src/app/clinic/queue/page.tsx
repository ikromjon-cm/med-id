...
const priorityColors: Record<string, { bg: string; text: string; dot: string; label: string }> = {
  ...
  MEDIUM: { bg: 'bg-amber-50 ', text: 'text-amber-500', dot: 'bg-amber-500', label: 'Medium' },
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 ">{t('Queue Management')}</h2>
          <p className="text-sm text-gray-500  mt-1">{t('Manage patient queue and priorities')}</p>
        </div>
        <motion.button
              ...
              return (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold text-gray-900 ">{entry.patientName}</p>
                  ...
                  <div className="flex items-center gap-1">
                    {entry.status === 'WAITING' && (
                      <>
                        ...
                        <button onClick={() => handleComplete(entry.id)} title={t('With Doctor')} className="w-9 h-9 rounded-lg flex items-center justify-center text-amber-500 hover:bg-amber-50  transition-all">
      ...
      <Modal isOpen={addModalOpen} onClose={() => setAddModalOpen(false)} title={t('Add to Queue')} size="md"
        footer={
          <>
            <button onClick={() => setAddModalOpen(false)} className="px-4 py-2 text-sm font-medium text-gray-600  hover:bg-gray-100  rounded-xl transition-all">{t('Cancel')}</button>
      ...
      >
        <div className="space-y-4">
          <div>
            ...
            <input type="text" value={formData.patientName} onChange={e => setFormData(prev => ({ ...prev, patientName: e.target.value }))} placeholder={t('Enter patient name')} className="w-full px-4 py-2.5 rounded-xl bg-gray-50  border border-gray-200  text-gray-900  placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all" />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700  mb-1">{t('Priority')}</label>
            <select value={formData.priority} onChange={e => setFormData(prev => ({ ...prev, priority: e.target.value as QueueEntry['priority'] }))} className="w-full px-4 py-2.5 rounded-xl bg-gray-50  border border-gray-200  text-gray-900  focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all">
