...
import {
  Search, User, Droplets, Phone, Calendar,
  ChevronRight
} from 'lucide-react';
...
export default function CRMPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold text-gray-900 dark:text-white">{t('Patient CRM')}</h2>
        <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">{t('View and manage patient relationships')}</p>
      </div>
      ...
      {loading ? (
        ...
        <EmptyState title={t('No patients found')} description={search ? t('Try a different search term.') : t('No patients registered yet.')} />
      ) : (
