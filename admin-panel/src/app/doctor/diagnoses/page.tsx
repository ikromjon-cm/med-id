...
export default function DiagnosesPage() {
  ...
  const columns: TableColumn<Diagnosis>[] = [
    ...
    },
    {
      ...
      render: (d: Diagnosis) => <span className="text-gray-500  truncate max-w-[250px] inline-block">{d.notes}</span>,
    },
  ];
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 ">{t('Diagnoses')}</h2>
          <p className="text-sm text-gray-500  mt-1">{t('Your diagnosis history')}</p>
        </div>
        <motion.button
      ...
      >
        <div className="space-y-4">
          ...
          <div>
            ...
            <input type="text" value={formData.condition} onChange={e => setFormData(p => ({ ...p, condition: e.target.value }))} className="w-full px-4 py-2.5 rounded-xl bg-gray-50  border border-gray-200  text-gray-900  placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all" placeholder={t('Diagnosed condition')} />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700  mb-1">{t('Notes')}</label>
            <textarea value={formData.notes} onChange={e => setFormData(p => ({ ...p, notes: e.target.value }))} rows={4} className="w-full px-4 py-2.5 rounded-xl bg-gray-50  border border-gray-200  text-gray-900  placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all resize-none" placeholder={t('Additional notes...')} />
          </div>
        </div>
      </Modal>
    </div>
