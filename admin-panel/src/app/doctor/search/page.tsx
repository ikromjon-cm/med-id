...
export default function PatientSearchPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold text-gray-900 ">{t('Patient Search')}</h2>
        <p className="text-sm text-gray-500  mt-1">{t('Search by name, MED-ID, or phone number')}</p>
      </div>
      ...
      {mode === 'search' ? (
        <>
          <div className="flex items-center gap-3">
            <div className="relative flex-1 max-w-xl">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-3 text-sm rounded-xl bg-white/70  border border-gray-200  text-gray-900  placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all"
              />
            </div>
            <motion.button
              whileHover={{ scale: 1.02 }}
            ...
            results.length === 0 ? (
              ...
              <div className="glass-card rounded-2xl overflow-hidden">
                <div className="px-6 py-3 border-b border-gray-100 ">
                  <p className="text-sm text-gray-500 ">{results.length} {results.length !== 1 ? t('results found') : t('result found')}</p>
                </div>
          ...
          {!searched && !searching && (
            <div className="glass-card rounded-2xl p-12 text-center">
              ...
              <h3 className="text-lg font-semibold text-gray-900  mb-1">{t('Search Patients')}</h3>
              <p className="text-sm text-gray-500  max-w-md mx-auto">{t('Enter a patient name, MED-ID, or phone number to find their profile.')}</p>
            </div>
          )}
        </>
      ) : (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          >
            <div className="relative w-64 h-64 rounded-2xl bg-gray-100  flex items-center justify-center mb-6 overflow-hidden">
            ...
            <p className="text-sm text-gray-500  text-center">{t('Position the QR code within the frame to scan. The camera will auto-detect MED-ID codes.')}</p>
          ...
          >
            <h3 className="text-base font-semibold text-gray-900  mb-2">{t('Manual Entry')}</h3>
            <p className="text-sm text-gray-500  mb-6">{t("Enter the patient's MED-ID number manually")}</p>
            ...
            <div className="flex items-center gap-3 mb-6">
              <div className="relative flex-1">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-4 py-3 text-sm rounded-xl bg-gray-50  border border-gray-200  text-gray-900  placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all"
                />
              </div>
              <motion.button
                whileHover={{ scale: 1.02 }}
            ...
            {qrPatient && (
              ...
              >
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-base font-semibold text-gray-900 ">{qrPatient.name}</p>
