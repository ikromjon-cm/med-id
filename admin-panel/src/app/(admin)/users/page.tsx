  ...
  return (
    <div className="space-y-6">
      ...
      <DataTable
        ...
        onEmptyAction={{ label: 'Add User', onClick: openCreate }}
      />
      ...
      >
        <div className="space-y-4">
          <FormInput label="Full Name" value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} error={formErrors.name} placeholder="John Doe" />
          <FormInput label="Email" type="email" value={form.email} onChange={e => setForm({ ...form, email: e.target.value })} error={formErrors.email} placeholder="john@medid.com" />
          <FormInput label="Role" as="select" value={form.role} onChange={e => setForm({ ...form, role: e.target.value as User['role'] })} options={ROLE_OPTIONS} />
          <FormInput label="Status" as="select" value={form.status} onChange={e => setForm({ ...form, status: e.target.value as User['status'] })} options={STATUS_OPTIONS} />
        </div>
      </Modal>
      ...
      >
        <p className="text-sm text-gray-600 dark:text-gray-400">
          Are you sure you want to delete <strong className="text-gray-900 dark:text-white">{deletingUser?.name}</strong>? This action cannot be undone.
        </p>
      </Modal>
    </div>
  );
