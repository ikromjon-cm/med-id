...
const ROLE_FILTERS = ['All', 'Admin', 'Doctor', 'Nurse', 'Receptionist', 'Patient'];
...
export default function AccessLogsPage() {
  ...
  const handleExport = () => {
    const csv = [['User', 'Role', 'Action', 'Resource', 'Timestamp', 'IP'].join(',')];
    logs.forEach(l => {
  ...
  const columns = [
    ...
    { key: 'timestamp', header: 'Timestamp', sortable: true, render: (l: AccessLog) => <span className="text-xs text-gray-500 dark:text-gray-400">{formatDateTime(l.timestamp)}</span> },
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-white">Access Logs</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Track all system access and activities</p>
        </div>
        <motion.button
      ...
      <DataTable
        ...
        emptyDescription={search || roleFilter !== 'All' || startDate ? 'Try adjusting your filters.' : 'No access logs recorded yet.'}
      />
    </div>
  );
}
