...
export default function NotificationsPage() {
  ...
  const validate = () => {
    ...
    if (form.recipientType === 'select' && form.recipients.length === 0) errors.recipients = 'Select at least one recipient';
    return errors;
  };
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Send and manage system notifications</p>
        </div>
        <motion.button
      ...
      {notifications.length === 0 ? (
        <EmptyState
          ...
          action={{ label: 'Send Notification', onClick: openCreate }}
        />
      ) : (
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-4 mt-3 text-xs text-gray-400 dark:text-gray-500">
                    ...
                    <span>{n.recipients.length === 1 && n.recipients[0] === 'all' ? 'All users' : `${n.recipients.length} recipient${n.recipients.length > 1 ? 's' : ''}`}</span>
                  </div>
                </div>
              </div>
            </motion.div>
      ...
      >
        <div className="space-y-4">
          <FormInput label="Type" as="select" value={form.type} onChange={e => setForm({ ...form, type: e.target.value as Notification['type'] })} options={NOTIF_TYPES} />
          <FormInput label="Title" value={form.title} onChange={e => setForm({ ...form, title: e.target.value })} error={formErrors.title} placeholder="Notification title" />
          <FormInput label="Message" as="textarea" value={form.message} onChange={e => setForm({ ...form, message: e.target.value })} error={formErrors.message} placeholder="Enter notification message..." />
