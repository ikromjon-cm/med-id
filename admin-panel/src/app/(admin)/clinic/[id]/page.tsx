  ...
  if (!data) {
    return (
      <div className="space-y-6">
        ...
        <EmptyState title="Clinic not found" description="The requested clinic could not be found." />
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard title="Doctors" value={doctors.length} icon={<Users className="w-5 h-5" />} delay={0} color="primary" subtitle={`${activeDoctors.length} active`} />
        <StatCard title="Queue" value={queue.filter(q => q.status !== 'COMPLETED').length} icon={<ListOrdered className="w-5 h-5" />} delay={0.1} color="amber" subtitle="Waiting" />
        <StatCard title="Today Appt." value={todayAppointments.length} icon={<Calendar className="w-5 h-5" />} delay={0.2} color="secondary" subtitle="Scheduled" />
        <StatCard title="Revenue" value={`$${(totalRevenue / 1000).toFixed(0)}k`} icon={<DollarSign className="w-5 h-5" />} delay={0.3} color="emergency" subtitle="Annual" />
      </div>
      ...
      <div className="flex items-center gap-3 flex-wrap">
        ...
        <button className="flex items-center gap-1.5 px-4 py-2 rounded-xl bg-amber-50 dark:bg-amber-500/10 text-amber-500 text-sm font-medium hover:bg-amber-100 dark:hover:bg-amber-500/20 transition-colors">
          <ListOrdered className="w-4 h-4" /> View Queue
        </button>
        <button className="flex items-center gap-1.5 px-4 py-2 rounded-xl bg-primary/10 text-primary text-sm font-medium hover:bg-primary/20 transition-colors">
          <BarChart3 className="w-4 h-4" /> View Finance
        </button>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <ChartCard title="Queue Status" subtitle="Current patient queue">
          <QueueStatus entries={queue} />
        </ChartCard>

        <ChartCard title="Finance Overview" subtitle="Monthly revenue and expenses">
          <ClinicFinanceChart data={finance} />
        </ChartCard>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <ChartCard title="Staff & Doctors" subtitle={`${doctors.length} doctors assigned`}>
          {doctors.length === 0 ? (
            <p className="text-sm text-gray-500 dark:text-gray-400 text-center py-6">No doctors assigned to this clinic</p>
          ) : (
        ...
        <ChartCard title="Today's Appointments" subtitle={`${todayAppointments.length} for today`}>
