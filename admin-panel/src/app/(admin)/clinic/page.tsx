...
import { Plus, Building2, Activity, Users, DollarSign } from 'lucide-react';
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage and monitor all clinics and their operations</p>
        </div>
        <motion.button
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          <StatCard title="Total Clinics" value={clinics.length} icon={<Building2 className="w-6 h-6" />} color="primary" delay={0} subtitle="All registered clinics" />
          <StatCard title="Active Clinics" value={activeClinics} icon={<Activity className="w-6 h-6" />} color="secondary" delay={0.1} subtitle="Currently operational" />
          <StatCard title="Total Doctors" value={totalDoctors} icon={<Users className="w-6 h-6" />} color="amber" delay={0.2} subtitle="Across all clinics" />
          <StatCard title="Today Revenue" value={`$${(Math.random() * 50000 + 20000).toFixed(0)}`} icon={<DollarSign className="w-6 h-6" />} color="emergency" delay={0.3} subtitle="Estimated" />
        </div>
      )}
