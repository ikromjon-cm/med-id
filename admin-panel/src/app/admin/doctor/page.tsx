...
export default function DoctorDashboardPage() {
  ...
  const columns = [
    ...
    { key: 'specialization', header: t('Specialization'), sortable: true, render: (d: Doctor) => <span className="px-2.5 py-0.5 text-xs font-medium rounded-full bg-primary/10 text-primary">{d.specialization}</span> },
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-white">{t('Doctor Management')}</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">{t('Manage and monitor all doctors across clinics')}</p>
        </div>
        <motion.button
      ...
      >
        <div className="space-y-4">
          ...
          <FormInput label={t('Specialization')} as="select" value={form.specialization} onChange={e => setForm({ ...form, specialization: e.target.value })} options={SPECIALIZATIONS.map(s => ({ value: s, label: s }))} error={formErrors.specialization} />
          <FormInput label={t('Clinic')} as="select" value={form.clinic} onChange={e => setForm({ ...form, clinic: e.target.value })} options={clinics.map(c => ({ value: c.name, label: c.name }))} error={formErrors.clinic} />
