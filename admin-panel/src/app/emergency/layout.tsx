...
const navItems = [
  { path: '/emergency/dashboard', label: t('Dashboard'), icon: LayoutDashboard },
  { path: '/emergency/alerts', label: t('Active Alerts'), icon: AlertTriangle },
  { path: '/emergency/qr-scanner', label: t('QR Scanner'), icon: Scan },
  { path: '/emergency/logs', label: t('Access Logs'), icon: ScrollText },
];

const pageTitles: Record<string, string> = {
  '/emergency/dashboard': t('Emergency Dashboard'),
  '/emergency/alerts': t('Emergency Alerts'),
  '/emergency/qr-scanner': t('QR Scanner'),
  '/emergency/logs': t('Access Logs'),
};
...
export default function EmergencyLayout({ children }: { children: React.ReactNode }) {
  ...
  const title = Object.entries(pageTitles).find(([path]) => pathname.startsWith(path))?.[1] || t('Emergency Staff Panel');
    ...
    <div className="flex flex-col h-full">
      <div className="flex items-center justify-between px-6 py-5 border-b border-gray-100 dark:border-gray-800/50">
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 rounded-xl bg-emergency flex items-center justify-center shadow-lg shadow-emergency/20">
            <Activity className="w-5 h-5 text-white" />
          </div>
          <div>
            <h1 className="text-base font-bold text-gray-900 dark:text-white tracking-tight">MED-ID</h1>
            <p className="text-[10px] text-gray-500 dark:text-gray-400 font-medium tracking-wider uppercase">{t('Emergency')}</p>
          </div>
        </div>
          ...
          {navItems.map((item) => {
            ...
            return (
              ...
              >
                ...
                {isActive && <motion.div layoutId="emergencyActiveTab" className="ml-auto w-1.5 h-1.5 rounded-full bg-white" />}
              </button>
            );
          })}
        </div>
  ...
  return (
    <div className="min-h-screen bg-bg-main dark:bg-[#0F0F15]">
      <aside className="hidden lg:flex lg:flex-col lg:w-64 lg:fixed lg:inset-y-0 z-30">
        <div className="flex-1 flex flex-col min-h-0 glass-card rounded-none border-r border-emergency/10 dark:border-emergency/10">
          {sidebarContent}
        </div>
      </aside>
      <AnimatePresence>
        {sidebarOpen && (
          <>
            ...
            <motion.aside initial={{ x: -300 }} animate={{ x: 0 }} exit={{ x: -300 }} transition={{ type: 'spring', damping: 25, stiffness: 250 }} className="fixed inset-y-0 left-0 z-50 w-64 lg:hidden">
              <div className="flex-1 flex flex-col min-h-0 h-full glass-card rounded-none border-r border-emergency/10">
                {sidebarContent}
              </div>
            </motion.aside>
          </>
      ...
      <div className="lg:pl-64 flex flex-col min-h-screen">
        <header className="sticky top-0 z-20 glass-card rounded-none border-b border-emergency/10 dark:border-emergency/10">
          <div className="flex items-center justify-between px-4 sm:px-6 py-3">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-xs text-gray-500 dark:text-gray-400 hidden sm:block">{t('MED-ID Emergency Response')}</p>
              </div>
            </div>
